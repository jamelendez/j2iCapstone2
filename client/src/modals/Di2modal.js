...
class Di2modal extends Component {
    ...
    onSubmit = (newName, newStatus, newAliasOFF, newAliasON) => {
        if (this.state.toAll) {
            ...
            const currentName = this.props.di1.di[1].name;
            const currentAliasOFF = this.props.di1.di[1].aliasOFF;
            const currentAliasON = this.props.di1.di[1].aliasON;
            console.log("currentName: " + currentName);
            if (newName == '') {
                newName = currentName
            }
