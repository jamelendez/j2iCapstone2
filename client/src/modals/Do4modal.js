...
class Do4modal extends Component {
    ...
    onSubmit = (e) => {
        ...
        if (this.state.toAll) {
            ...
            this.props.setDOChannelName(this.state.name, 4);
            this.setState({ nameChanged: false });
            if (this.state.aliasOFF !== '') this.props.setDOChannelAliasOFF(this.state.aliasOFF, 4);
            if (this.state.aliasON !== '') this.props.setDOChannelAliasON(this.state.aliasON, 4);
            if (this.state.status === false) {
                this.props.setDOChannelStatus(this.state.aliasOFF, 4);
            }
            else {
                this.props.setDOChannelStatus(this.state.aliasON, 4);
            }
        }
    ...
    render() {
        ...
        const { name } = this.props.do1.do.find(channel => channel.ch === 4);
